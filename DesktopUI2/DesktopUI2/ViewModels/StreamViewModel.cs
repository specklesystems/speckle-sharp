...
public class StreamViewModel : ReactiveObject, IRoutableViewModel, IDisposable
{
  public StreamViewModel(StreamState streamState, IScreen hostScreen, ICommand removeSavedStreamCommand)
  {
    ...
    {
      SpeckleLog.Logger.ForContext("StreamState", streamState).Fatal(ex, "Failed to create stream view model");
    }
  }
  ...
  private async void AddNewBranch()
  {
    ...
    if (result)
      try
      {
        ...
        var branchId = await StreamState.Client.BranchCreate(
  ...
  public async void CopyReportCommand()
  {
    ...
    await Application.Current.Clipboard.SetTextAsync(summary);
  ...
  public async void PreviewCommand()
  {
    ...
    try
    {
      ...
      GetReport();
      SpeckleLog.Logger
        .ForContext("IsReceiver", IsReceiver)
