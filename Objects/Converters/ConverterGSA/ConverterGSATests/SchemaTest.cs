...
namespace ConverterGSATests
{
  public class SchemaTest : SpeckleConversionFixture
  {
    ...
    public void GsaAxis()
    {
      ...
      var axis = (Axis)structuralObjects.FirstOrDefault(so => so is Axis);

      Assert.Equal("axis 1", axis.applicationId);
      Assert.Equal(AxisType.Cartesian, axis.axisType);
      Assert.Equal(gsaAxis.OriginX, axis.definition.origin.x);
      Assert.Equal(gsaAxis.OriginY, axis.definition.origin.y);
      Assert.Equal(gsaAxis.OriginZ, axis.definition.origin.z);
      Assert.Equal(gsaAxis.XDirX, axis.definition.xdir.x);
      Assert.Equal(gsaAxis.XDirY, axis.definition.xdir.y);
      Assert.Equal(gsaAxis.XDirZ, axis.definition.xdir.z);
      Assert.Equal(gsaAxis.XYDirX, axis.definition.ydir.x);
      Assert.Equal(gsaAxis.XYDirY, axis.definition.ydir.y);
      Assert.Equal(gsaAxis.XYDirZ, axis.definition.ydir.z);
    }
    ...
    public void GsaNode()
    {
      ...
      Assert.Equal("node 1", node.applicationId);
    ...
    public void GsaElement2D()
    {
      ...
      //var speckleElement2D = (Element2D)structuralObjects.FirstOrDefault(so => so is Element2D);
    ...
    public void GsaProperty2D()
    {
      ...
      gsaModelMock.NativesByKeywordId = new Dictionary<GwaKeyword, Dictionary<int, GsaRecord>>
      {
        ...
        { GwaKeyword.AXIS, new Dictionary<int, GsaRecord>
          { { 1, gsaAxis } }
        }
      };
      gsaModelMock.IndicesByKeyword = new Dictionary<GwaKeyword, List<int>>
      {
        { GwaKeyword.MAT_STEEL, new List<int> { 1 } },
        { GwaKeyword.AXIS, new List<int> { 1 } }
      };
      ...
      Assert.Equal(gsaMatSteel.ApplicationId, speckleProperty2D.material.applicationId);
      Assert.Equal("", speckleProperty2D.material.designCode);
      Assert.Equal("", speckleProperty2D.material.codeYear);
      Assert.Equal(gsaMatSteel.Fy.Value, (speckleProperty2D.material as Steel).yieldStrength);
      Assert.Equal(gsaMatSteel.Fu.Value, (speckleProperty2D.material as Steel).ultimateStrength);
      Assert.Equal(gsaMatSteel.EpsP.Value, (speckleProperty2D.material as Steel).maxStrain);
      Assert.Equal(gsaMatSteel.Mat.E.Value, (speckleProperty2D.material as Steel).youngsModulus);
      Assert.Equal(gsaMatSteel.Mat.Nu.Value, (speckleProperty2D.material as Steel).poissonsRatio);
      Assert.Equal(gsaMatSteel.Mat.G.Value, (speckleProperty2D.material as Steel).shearModulus);
      Assert.Equal(gsaMatSteel.Mat.Rho.Value, (speckleProperty2D.material as Steel).density);
      Assert.Equal(gsaMatSteel.Mat.Alpha.Value, (speckleProperty2D.material as Steel).thermalExpansivity);
      
      //Check axis properties
      Assert.Equal(AxisType.Cartesian, speckleProperty2D.orientationAxis.axisType);
      Assert.Equal(gsaAxis.OriginX, speckleProperty2D.orientationAxis.definition.origin.x);
      Assert.Equal(gsaAxis.OriginY, speckleProperty2D.orientationAxis.definition.origin.y);
      Assert.Equal(gsaAxis.OriginZ, speckleProperty2D.orientationAxis.definition.origin.z);
      Assert.Equal(0, speckleProperty2D.orientationAxis.definition.normal.x);
      Assert.Equal(0, speckleProperty2D.orientationAxis.definition.normal.y);
      Assert.Equal(1, speckleProperty2D.orientationAxis.definition.normal.z);
      Assert.Equal(1 / Math.Sqrt(2), speckleProperty2D.orientationAxis.definition.xdir.x, 6);
      Assert.Equal(1 / Math.Sqrt(2), speckleProperty2D.orientationAxis.definition.xdir.y, 6);
      Assert.Equal(0, speckleProperty2D.orientationAxis.definition.xdir.z);
      Assert.Equal(-1 / Math.Sqrt(2), speckleProperty2D.orientationAxis.definition.ydir.x, 6);
      Assert.Equal(1 / Math.Sqrt(2), speckleProperty2D.orientationAxis.definition.ydir.y, 6);
      Assert.Equal(0, speckleProperty2D.orientationAxis.definition.ydir.z);

      //Check remaining 2d properties
      Assert.Equal(gsaProp2d.ApplicationId, speckleProperty2D.applicationId);
    ...
    private GsaPropSpr GsaPropSprExample()
    {
      ...
      {
        Index = 1,
        Stiffnesses = new Dictionary<AxisDirection6, double> { { AxisDirection6.XX, 10 } }
      };
    }
    ...
    private List<GsaEl> GsaElementExample(int numberOfElements)
    {
      ...
      return gsaEl.GetRange(0, numberOfElements - 1);
    }
    #endregion
  }
}
