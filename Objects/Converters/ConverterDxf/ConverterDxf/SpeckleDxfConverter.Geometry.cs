...
namespace Objects.Converters.DxfConverter
{
  public partial class SpeckleDxfConverter
  {
    ...
    public Dxf.Vector3 VectorToNative(Vector pt) =>
      new Dxf.Vector3(ScaleToNative(pt.x, pt.units), ScaleToNative(pt.y, pt.units), ScaleToNative(pt.z, pt.units));
    ...
    public Dxfe.Mesh MeshToNative(Mesh mesh) =>
      new Dxfe.Mesh(mesh.GetPoints().Select(VectorToNative), mesh.GetFaceIndices())
      {
    ...
    public IEnumerable<Dxfe.EntityObject> MeshToNativePretty(Mesh mesh, RenderMaterial renderMaterial = null)
    {
      ...
      return MeshFacesToNative(topology, renderMaterial: renderMaterial ?? mesh["renderMaterial"] as RenderMaterial)
    ...
    private IEnumerable<Dxfe.Line> MeshEdgesToNative(
      ...
      topology.EdgeFaceConnection
        ...
        {
          var (iA, iB) = kv.Key;
          var line = new Dxfe.Line(VectorToNative(topology.Vertices[iA]), VectorToNative(topology.Vertices[iB]));
