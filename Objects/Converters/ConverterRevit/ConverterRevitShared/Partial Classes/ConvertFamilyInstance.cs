...
namespace Objects.Converter.Revit
{
  public partial class ConverterRevit
  {
    ...
    public Base FamilyInstanceToSpeckle(DB.FamilyInstance revitFi, out List<string> notes)
    {
      ...
      if (@base == null)
      {
        if (revitFi.HasModifiedGeometry())
        {
    ...
    private Base PointBasedFamilyInstanceToSpeckle(DB.FamilyInstance revitFi, Point basePoint, out List<string> notes)
    {
      ...
      speckleFi.placementType = symbol?.Family?.FamilyPlacementType.ToString();
      //TODO - do we need to change this transform for nested family instances
      speckleFi.transform = TransformToSpeckle(revitFi.GetTotalTransform(), revitFi.Document, false);
