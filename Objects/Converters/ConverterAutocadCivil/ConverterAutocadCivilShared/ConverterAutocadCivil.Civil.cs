...
public partial class ConverterAutocadCivil
{
  ...
  public CivilAlignment AlignmentToSpeckle(CivilDB.Alignment alignment)
  {
    ...
    {
      equations.AddRange(
        new List<double> { stationEquation.RawStationBack, stationEquation.StationBack, stationEquation.StationAhead }
      );
    ...
    CivilAlignment speckleAlignment =
      new()
      {
    ...
    if (alignment.IsOffsetAlignment)
    {
      ...
      if (
        Trans.GetObject(offsetInfo.ParentAlignmentId, OpenMode.ForRead) is CivilDB.Alignment parent
        && parent.Name != null
      )
      {
  ...
  {
    var appObj = new ApplicationObject(alignment.id, alignment.speckle_type)
    {
      applicationId = alignment.applicationId
    };
    ...
    CivilDB.Alignment existingAlignment = existingObjs.Any()
      ? Trans.GetObject(existingObjs.FirstOrDefault(), OpenMode.ForWrite) as CivilDB.Alignment
      : null;
    var parent =
      civilAlignment != null
        ? GetFromObjectIdCollection(civilAlignment.parent, civilDoc.GetAlignmentIds())
        : ObjectId.Null;
    bool isUpdate = true;
    if (existingAlignment == null || ReceiveMode == Speckle.Core.Kits.ReceiveMode.Create) // just create a new alignment
    {
      ...
      // site
      var site =
        civilAlignment != null ? GetFromObjectIdCollection(civilAlignment.site, civilDoc.GetSiteIds()) : ObjectId.Null;
      ...
      var style =
        civilAlignment != null
          ? GetFromObjectIdCollection(civilAlignment.style, docStyles, true)
      ...
      var label =
        civilAlignment != null
          ? GetFromObjectIdCollection(civilAlignment["label"] as string, labelStyles, true)
      ...
      switch (type)
      {
        case AlignmentType.Offset:
          ...
          catch (ArgumentException) // throws when name is invalid or offset is too large
          {
            ...
            try
            {
              id = CivilDB.Alignment.CreateOffsetAlignment(
                CivilDB.Alignment.GetNextUniqueName(name),
                parent,
                civilAlignment.offset,
                style
              );
            }
  ...
  private void AddAlignmentEntity(ICurve curve, ref CivilDB.AlignmentEntityCollection entities)
  {
    switch (curve)
    {
      ...
      case Arc o:
        entities.AddFixedCurve(
          entities.LastEntity,
          PointToNative(o.startPoint),
          PointToNative(o.midPoint),
          PointToNative(o.endPoint)
        );
        break;
  ...
  private Spiral AlignmentSpiralToSpeckle(AlignmentSubEntitySpiral spiral, CivilDB.Alignment alignment)
  {
    // get plane
    var vX = new Vector3d(
      Math.Cos(spiral.StartDirection) + spiral.StartPoint.X,
      Math.Sin(spiral.StartDirection) + spiral.StartPoint.Y,
      0
    );
    ...
    List<Point2d> points = new() { spiral.StartPoint };
  ...
  public CivilProfile ProfileToSpeckle(CivilDB.Profile profile)
  {
    ...
    for (int i = 0; i < profile.Entities.Count; i++)
    {
      ...
      switch (entity.EntityType)
      {
        ...
        default:
          var segment = ProfileGenericToSpeckle(
            entity.StartStation,
            entity.StartElevation,
            entity.EndStation,
            entity.EndElevation
          );
          if (segment != null)
    ...
    {
      if (
        Trans.GetObject(profile.OffsetParameters.ParentProfileId, OpenMode.ForRead) is CivilDB.Profile parent
        && parent.Name != null
      )
      {
  ...
  private Line ProfileGenericToSpeckle(
    double startStation,
    double startElevation,
    double endStation,
    double endElevation
  ) // general approximation of segment as line
  {
  ...
  public Featureline FeaturelineToSpeckle(CivilDB.FeatureLine featureline)
  {
    ...
    // featureline
    Featureline speckleFeatureline =
      new()
      {
        points = points,
  ...
  public Mesh SurfaceToSpeckle(TinSurface surface)
  {
    ...
    var mesh = new Mesh(vertices, faces) { units = ModelUnits, bbox = BoxToSpeckle(surface.GeometricExtents) };
  ...
  public Mesh SurfaceToSpeckle(GridSurface surface)
  {
    ...
    foreach (var cell in surface.GetCells(false))
    {
      ...
      foreach (
        var vertex in new List<GridSurfaceVertex>()
        {
  ...
  public ApplicationObject TinSurfaceToNative(Mesh mesh, Base props)
  {
    ...
    CivilDB.TinSurface surface = existingObjs.Any()
      ? Trans.GetObject(existingObjs.FirstOrDefault(), OpenMode.ForWrite) as CivilDB.TinSurface
      : null;
    bool isUpdate = true;
    ...
    foreach (TinSurfaceVertex vertex in surface.Vertices)
    {
      if (vertex.Edges.Count > edges[vertex.Location].Count)
      {
        ...
        {
          if (
            !edges[vertex.Location].Contains(modifiedEdge.Vertex2.Location)
            && !edges[modifiedEdge.Vertex2.Location].Contains(vertex.Location)
          )
          {
  ...
  public Structure StructureToSpeckle(CivilDB.Structure structure)
  {
    ...
    try
    {
      speckleStructure["rotation"] = structure.Rotation;
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
      speckleStructure["sumpDepth"] = structure.SumpDepth;
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
      speckleStructure["rimElevation"] = structure.RimElevation;
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
      speckleStructure["sumpElevation"] = structure.SumpElevation;
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
      speckleStructure["lengthOuter"] = structure.Length;
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
      speckleStructure["lengthInner"] = structure.InnerLength;
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
      speckleStructure["structureId"] = structure.Id.ToString();
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
  ...
  private Base PartDataRecordToSpeckle(PartDataRecord partData)
  {
    ...
    {
      CivilDataField field =
        new(
          partField.Name,
          partField.DataType.ToString(),
          partField.Value,
          partField.Units.ToString(),
          partField.Context.ToString(),
          null
        );
  ...
  private List<CivilDataField> PartDataRecordToSpeckle(PressureNetworkPartData partData)
  {
    ...
    {
      CivilDataField field =
        new(partField.Name, partField.GetType().ToString(), partField.Value, null, null, partField.DisplayName);
      fields.Add(field);
    }
  ...
  public Pipe PipeToSpeckle(CivilDB.Pipe pipe)
  {
    ...
    try
    {
      specklePipe["shape"] = pipe.CrossSectionalShape.ToString();
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["slope"] = pipe.Slope;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["flowDirection"] = pipe.FlowDirection.ToString();
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["flowRate"] = pipe.FlowRate;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["network"] = pipe.NetworkName;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["startOffset"] = pipe.StartOffset;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["endOffset"] = pipe.EndOffset;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["startStation"] = pipe.StartStation;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["endStation"] = pipe.EndStation;
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["startStructureId"] = pipe.StartStructureId.ToString();
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["endStructureId"] = pipe.EndStructureId.ToString();
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
    try
    {
      specklePipe["pipeId"] = pipe.Id.ToString();
    }
    catch (Exception ex) when (!ex.IsFatal()) { }
  ...
  public Pipe PipeToSpeckle(PressurePipe pipe)
  {
    ...
    try
    {
      specklePipe["partType"] = pipe.PartType.ToString();
    }
    catch (Exception e) when (!e.IsFatal()) { }
    try
    {
  ...
  {
    CivilDataField baseParam =
      new(param.KeyName, param.ValueType.Name, param.ValueAsObject, null, null, param.DisplayName);
  ...
  private CivilAppliedSubassembly AppliedSubassemblyToSpeckle(AppliedSubassembly appliedSubassembly)
  {
    ...
    List<CivilDataField> speckleParameters = appliedSubassembly
      .Parameters.Select(p => AppliedSubassemblyParamToSpeckle(p))
      .ToList();

    CivilAppliedSubassembly speckleAppliedSubassembly =
      new(appliedSubassembly.SubassemblyId.ToString(), subassembly.Name, speckleShapes, soePoint, speckleParameters)
      {
        units = ModelUnits
      };
  ...
  private CivilBaselineRegion BaselineRegionToSpeckle(BaselineRegion region)
  {
    ...
    CivilBaselineRegion speckleRegion =
      new(
        region.Name,
        region.StartStation,
        region.EndStation,
  ...
  private CivilCalculatedLink CalculatedLinkToSpeckle(CalculatedLink link)
  {
    ...
    CivilCalculatedLink speckleLink = new(codes, specklePoints) { units = ModelUnits };
  ...
  private CivilCalculatedPoint CalculatedPointToSpeckle(CalculatedPoint point)
  {
    ...
    CivilCalculatedPoint speckleCalculatedPoint =
      new(specklePoint, codes, normalBaseline, normalSubAssembly, soePoint) { units = ModelUnits };
  ...
  private CivilBaseline BaselineToSpeckle(CivilDB.Baseline baseline)
  {
    ...
    if (!baseline.IsFeatureLineBased())
    {
      ...
      speckleBaseline = new(
        baseline.Name,
        speckleRegions,
        baseline.SortedStations().ToList(),
