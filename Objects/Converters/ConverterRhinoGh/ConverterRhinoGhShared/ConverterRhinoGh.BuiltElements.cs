...
public partial class ConverterRhinoGh
{
  ...
  public Duct CurveToSpeckleDuct(RH.Curve curve, string[] args, out List<string> notes)
  {
    ...
    if (
      ...
      && double.TryParse(args[2], out double diameter)
    )
      duct = new Duct(CurveToSpeckle(curve), width, height, diameter)
      {
        units = ModelUnits,
  ...
  public Wall BrepToSpeckleWall(RH.Brep brep, out List<string> notes)
  {
    ...
    RH.BoundingBox brepBox = brep.GetBoundingBox(false);
  ...
  public RV.AdaptiveComponent InstanceToAdaptiveComponent(InstanceObject instance, string[] args)
  {
    ...
    var points = instance.GetSubObjects().Select(o => PointToSpeckle(((RH.Point)o.Geometry).Location)).ToList();
