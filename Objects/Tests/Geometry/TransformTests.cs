...
public class TransformTests
{
  ...
  public void ArrayBackAndForth(Matrix4X4<double> data)
  {
  ...
  [Test, TestCaseSource(nameof(TransformTestCases))]
  public void ConvertToUnits(Matrix4X4<double> data)
  {
    ...
    Assert.Multiple(() =>
    {
      ...
      Assert.That(Quaternion<double>.Dot(mr, mmr), Is.LessThan(1).Within(FLOAT_TOLLERANCE), "Expect rot x to be equivalent");
  ...
  public void Decompose(Matrix4X4<double> data)
  {
    var transpose = Matrix4X4.Transpose(data); //NOTE: Transform expects matrices transposed (translation in column 4)
    ...
    var t = new Vector3D<double>(_t.X, _t.Y, _t.Z);
    ...
    Assert.Multiple(() =>
    {
      ...
      Assert.That(
        Quaternion<double>.Dot(r, expectedR),
  ...
  {
    var t = new Vector3D<double>(128.128f, 255.255f, 512.512f);
    var r = Quaternion<double>.CreateFromYawPitchRoll(1.9f, 0.6666667f, 0.5f);
    var s = new Vector3D<double>(123f, 32f, 0.5f);

    yield return new TestCaseData(Matrix4X4<double>.Identity).SetName("{m} Identity Matrix");

    yield return new TestCaseData(Matrix4X4.CreateTranslation(t)).SetName("{m} Translation Only (positive)");

    yield return new TestCaseData(Matrix4X4.CreateTranslation(t * -Vector3D<double>.UnitX)).SetName("{m} Translation Only -X");

    yield return new TestCaseData(Matrix4X4.CreateTranslation(t * -Vector3D<double>.UnitY)).SetName("{m} Translation Only -Y");

    yield return new TestCaseData(Matrix4X4.CreateTranslation(t * -Vector3D<double>.UnitZ)).SetName("{m} Translation Only -Z");

    yield return new TestCaseData(Matrix4X4.CreateTranslation(-t)).SetName("{m} Translation Only -XYZ ");

    yield return new TestCaseData(Matrix4X4.CreateFromYawPitchRoll(0.5, 0.0, 0.0)).SetName("{m} Rotation Only X ");

    yield return new TestCaseData(Matrix4X4.CreateFromYawPitchRoll(0.0, 0.5, 0.0)).SetName("{m} Rotation Only Y ");

    yield return new TestCaseData(Matrix4X4.CreateFromYawPitchRoll(0.0, 0.0, 0.5)).SetName("{m} Rotation Only Z ");

    yield return new TestCaseData(Matrix4X4.CreateFromYawPitchRoll(0.5, 0.5, 0.5)).SetName("{m} Rotation Only XYZ ");

    yield return new TestCaseData(Matrix4X4.CreateFromQuaternion(r)).SetName("{m} Rotation Only");

    yield return new TestCaseData(Matrix4X4<double>.Identity + Matrix4X4.CreateScale(s)).SetName("{m} Scale Only");
