...
public class SchemaObjectFilter
{
  ...
  public SchemaObjectFilter(List<RhinoObject> docObjects, RhinoDoc doc, string inputSchema = null)
  {
    Doc = doc;
  ...
  public Dictionary<string, List<RhinoObject>> SchemaDictionary = new();
  public double minDimension =
    25 * Units.GetConversionFactor(Units.Millimeters, RhinoDoc.ActiveDoc.ModelUnitSystem.ToString());
  ...
  private void ApplyGeomFilter(List<RhinoObject> objs, string inputSchema = null)
  {
    if (inputSchema != null)
    {
      if (Enum.TryParse(inputSchema, out SupportedSchema schema))
        foreach (RhinoObject obj in objs)
          if (IsViableSchemaObject(schema, obj))
            SchemaDictionary[inputSchema].Add(obj);
    }
    else
    {
      foreach (RhinoObject obj in objs)
      {
        ...
        foreach (var testSchema in schemasToTest)
          if (IsViableSchemaObject(testSchema, obj))
            SchemaDictionary[testSchema.ToString()].Add(obj);
        break;
      }
    }
  }
    ...
    switch (type)
    {
      case ObjectType.Curve:
        ...
        if (crv.IsLinear()) // test for linearity
          objSchemas = new List<SupportedSchema> { SupportedSchema.Beam, SupportedSchema.Column };
        break;
      ...
      case ObjectType.Extrusion:
        objSchemas = new List<SupportedSchema> { SupportedSchema.Topography };
        break;
    }
    return objSchemas;
  }
