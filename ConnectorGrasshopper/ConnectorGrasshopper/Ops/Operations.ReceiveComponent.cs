...
namespace ConnectorGrasshopper.Ops
{
  ...
  //public class ReceiveComponent2Worker : WorkerInstance
  //{
  //  private GH_Structure<IGH_Goo> DataInput;
  //  private Action<string, Exception> ErrorAction;
  ...
  //  public ReceiveComponent2Worker(GH_Component p) : base(p)
  //  {
  //  }
  ...
  //  public override WorkerInstance Duplicate()
  //  {
  //    return new ReceiveComponent2Worker(Parent);
  //  }
  ...
  //    ((ReceiveSync)Parent).CurrentComponentState = "up_to_date";

  //    if (ReceivedCommit != null)
  //    {
  //      ((ReceiveSync)Parent).LastInfoMessage =
  //        $"{ReceivedCommit.authorName} @ {ReceivedCommit.createdAt}: {ReceivedCommit.message} (id:{ReceivedCommit.id})";

  //      ((ReceiveSync)Parent).ReceivedCommitId = ReceivedCommit.id;
  //    }

  //    ((ReceiveSync)Parent).JustPastedIn = false;

  //    DA.SetData(1, ((ReceiveSync)Parent).LastInfoMessage);

  //    if (ReceivedObject == null) return;

  //    ((ReceiveSync)Parent).ReceivedObjectId = ReceivedObject.id;

  //    //the active document may have changed
  //    ((ReceiveSync)Parent).Converter.SetContextDocument(RhinoDoc.ActiveDoc);

  //    // case 1: it's an item that has a direct conversion method, eg a point
  //    if (((ReceiveSync)Parent).Converter.CanConvertToNative(ReceivedObject))
  //    {
  //      DA.SetData(0, Utilities.TryConvertItemToNative(ReceivedObject, ((ReceiveSync)Parent).Converter));
  //      return;
  //    }
