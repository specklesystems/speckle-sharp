...
public class VariableInputReceiveComponent : SelectKitAsyncComponentBase, IGH_VariableParameterComponent
{
  ...
  public VariableInputReceiveComponent()
    : base(
      "Receive",
      "Receive",
      "Receive data from a Speckle server",
  ...
  public override Guid ComponentGuid => new("06A3E53B-2BFF-4EBD-BBCE-71B9CE36283E");
  ...
  public override void AppendAdditionalMenuItems(ToolStripDropDown menu)
  {
    ...
    var noExpandMi = Menu_AppendItem(
      menu,
      "Expand commit object properties",
      (s, e) =>
      {
    ...
    receivOnOpenMi.ToolTipText =
      "The node will automatically perform a receive operation as soon as the document is open, or the node is copy/pasted into a new document.";
  ...
  protected override void SolveInstance(IGH_DataAccess DA)
  {
    ...
    if (
      (AutoReceive || CurrentComponentState == "primed_to_receive" || CurrentComponentState == "receiving")
      && !JustPastedIn
    )
    {
    ...
    else
    {
      ...
      if (PrevReceivedData != null)
      {
        ...
        AddRuntimeMessage(
          ...
          "Output is based on a prior receive operation. If you are seeing this message, you most likely recomputed the Grasshopper solution (F5). To ensure you have the latest data, press the Receive button again."
        );
      }
      OnDisplayExpired(true);
    }
...
public class VariableInputReceiveComponentWorker : WorkerInstance
{
  ...
  public override void DoWork(Action<string, double> ReportProgress, Action Done)
  {
    ...
    try
    {
      ...
      var t = Task.Run(async () =>
      {
        ...
        try
        {
          await receiveComponent.ApiClient.CommitReceived(
  ...
  public static async Task<Commit> GetCommit(
    StreamWrapper InputWrapper,
    Client client,
    Action<GH_RuntimeMessageLevel, string> OnFail,
  ...
  {
    return outputList.Count != Parent.Params.Output.Count
  ...
  private void AutoCreateOutputs(Base @base)
  {
    ...
    if (HasSingleRename())
    {
      ...
      var diffOut = outputList.Where(name => !Parent.Params.Output.Select(p => p.Name).Contains(name));
    ...
    outputList.ForEach(s =>
    {
      ...
      {
        var newParam =
          ((IGH_VariableParameterComponent)Parent).CreateParameter(GH_ParameterSide.Output, Parent.Params.Output.Count)
          as SendReceiveDataParam;
        newParam.Name = name;
...
public class VariableInputReceiveComponentAttributes : GH_ComponentAttributes
{
  ...
  public VariableInputReceiveComponentAttributes(GH_Component owner)
    : base(owner) { }
