  ...
  public class GsaMatConcreteParser : GwaParser<GsaMatConcrete>
  {
    ...
    public override bool FromGwa(string gwa)
    {
      ...
      return FromGwaByFuncs(remainingItems, out var _, (v) => Enum.TryParse<MatConcreteType>(v, true, out record.Type), (v) => Enum.TryParse<MatConcreteCement>(v, true, out record.Cement),
        ...
        (v) => AddNullableDoubleValue(v, out record.N), (v) => AddNullableDoubleValue(v, out record.Emod), (v) => AddNullableDoubleValue(v, out record.EpsPeak),
        (v) => AddNullableDoubleValue(v, out record.EpsMax), (v) => AddNullableDoubleValue(v, out record.EpsU), (v) => AddNullableDoubleValue(v, out record.EpsAx),
        (v) => AddNullableDoubleValue(v, out record.EpsTran), (v) => AddNullableDoubleValue(v, out record.EpsAxs), AddLight,
        (v) => AddNullableDoubleValue(v, out record.Agg), null, (v) => AddNullableDoubleValue(v, out record.XdMin), (v) => AddNullableDoubleValue(v, out record.XdMax),
    ...
    public override bool Gwa(out List<string> gwa, bool includeSet = false)
    {
      ...
      //MAT_CONCRETE | num | <mat> | type | cement | fc | fcd | fcdc | fcdt | fcfib | EmEs | n | Emod | eps_peak | eps_max | eps_u | eps_ax | eps_tran | eps_axs | light | agg | 0.0 | xd_min | xd_max | beta | shrink | confine | fcc | eps_plas_c |eps_u_c
      AddMat(ref items);
      AddItems(ref items, record.Type, record.Cement, record.Fc, record.Fcd, record.Fcdc, record.Fcdt, record.Fcfib, record.EmEs, record.N, record.Emod, record.EpsPeak, record.EpsMax, record.EpsU, record.EpsAx, record.EpsTran, record.EpsAxs, record.Light, record.Agg, 0.0, record.XdMin, record.XdMax,
        record.Beta, record.Shrink, record.Confine, record.Fcc, record.EpsPlasC, record.EpsUC);
