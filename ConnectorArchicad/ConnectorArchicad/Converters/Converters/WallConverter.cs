...
namespace Archicad.Converters
{
  public sealed class Wall : IConverter
  {
    ...
    public async Task<List<string>> ConvertToArchicad(IEnumerable<Base> elements, CancellationToken token)
    {
      ...
      IEnumerable<string> result = await AsyncCommandProcessor.Execute(new Communication.Commands.CreateWall(walls), token);
    ...
    {
      IEnumerable<Objects.BuiltElements.Archicad.Wall> datas = await AsyncCommandProcessor.Execute(new Communication.Commands.GetWallData(elements.Select(e => e.elementId)), token);
      if (datas is null)
      {
