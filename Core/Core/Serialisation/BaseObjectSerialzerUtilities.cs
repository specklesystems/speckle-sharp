...
internal static class SerializationUtilities
{
  ...
  )
  {
        ...
        foreach (var val in (JArray)value)
        {
            ...
            foreach (var dataItem in chunk.data)
              if (hasGenericType && !jsonProperty.PropertyType.GenericTypeArguments[0].IsInterface)
              {
                if (jsonProperty.PropertyType.GenericTypeArguments[0].IsAssignableFrom(dataItem.GetType()))
                  ...
                  addMethod.Invoke(
                    arr,
                    new object[] { Convert.ChangeType(dataItem, jsonProperty.PropertyType.GenericTypeArguments[0]) }
                  );
              }
              else
              {
          ...
          else if (hasGenericType && !jsonProperty.PropertyType.GenericTypeArguments[0].IsInterface)
          {
            if (jsonProperty.PropertyType.GenericTypeArguments[0].IsAssignableFrom(item.GetType()))
              ...
              addMethod.Invoke(
                arr,
                new object[] { Convert.ChangeType(item, jsonProperty.PropertyType.GenericTypeArguments[0]) }
              );
          }
          else
          {
    ...
    if (value is JObject)
    {
      if (((JObject)value).Property(TypeDiscriminator) != null) return value.ToObject<Base>(serializer);
      ...
      foreach (var prop in (JObject)value)
      {
        ...
        object key = prop.Key;
        if (jsonProperty != null) key = Convert.ChangeType(prop.Key, jsonProperty.PropertyType.GetGenericArguments()[0]);
  ...
  internal static Type GetType(string objFullType)
  {
    lock (cachedTypes)
    {
      if (cachedTypes.ContainsKey(objFullType)) return cachedTypes[objFullType];
  ...
  {
    if (cachedAbstractTypes.ContainsKey(assemblyQualifiedName)) return jToken.ToObject(cachedAbstractTypes[assemblyQualifiedName]);
    ...
    if (myAssembly == null) throw new SpeckleException("Could not load abstract object's assembly.");

    var myType = myAssembly.GetType(pieces[0]);
    if (myType == null) throw new SpeckleException("Could not load abstract object's assembly.");
