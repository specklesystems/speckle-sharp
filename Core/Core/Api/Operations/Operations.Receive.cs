      ...
      using (LogContext.PushProperty("objectId", objectId))
      {
        ...
        if (objString != null)
        {
          ...
          else
          {
            ...
            catch (Exception ex)
            {
              ...
              serializerV2.OnErrorAction.Invoke(
                ...
                new SpeckleException($"A deserialization error has occurred: {ex.Message}", ex)
              );
              localRes = null;
            }
          }
          ...
          if (
            ...
            && remoteTransport is IDisposable dispRemote
          )
            dispRemote.Dispose();
        ...
        objString = await remoteTransport.CopyObjectAndChildren(
          objectId,
          localTransport,
          onTotalChildrenCountKnown
        );
        ...
        if (
          (disposeTransports || !hasUserProvidedLocalTransport) && localTransport is IDisposable dl
        )
          dl.Dispose();
